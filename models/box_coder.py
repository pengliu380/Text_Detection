...
from utils.encode import meshgrid
from utils.encode.box import box_iou, box_nms, change_box_order


class SSDBoxCoder:
    def __init__(self, ssd_model):
        self.steps = ssd_model.steps # steps = (8, 16, 32, 64, 100, 300)
        self.box_sizes = ssd_model.box_sizes # box_sizes = (30, 60, 111, 162, 213, 264, 315)
        self.aspect_ratios = ssd_model.aspect_ratios # 2,3,5,7,10
        self.fm_sizes = ssd_model.fm_sizes # fm_sizes = (38, 19, 10, 5, 3, 1)
    ...
    def _get_default_boxes(self):
        ...
        for i, fm_size in enumerate(self.fm_sizes):
            for h, w in itertools.product(range(fm_size), repeat=2):
                ...
                cy_offset = (h + 1.0) * self.steps[i]

                s = self.box_sizes[i]
                boxes.append((cx, cy, s, s))
                boxes.append((cx, cy_offset, s, s))
                ...
                for ar in self.aspect_ratios[i]:
                    ...
                    boxes.append((cx, cy_offset, s * math.sqrt(ar), s / math.sqrt(ar)))
    ...
    def encode(self, boxes, labels):
        ...
        #print(mask[0])
        if mask.any():
            index[mask] = ious[mask.nonzero().squeeze(-1)].max(1)[1]
